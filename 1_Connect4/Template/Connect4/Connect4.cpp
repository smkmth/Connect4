...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	HANDLE* phwndConsole = &g_hwndConsole;



	// initialise board class
	CGameBoard cCurrentGameBoard(phwndConsole);
	CGameBoard pCurrentGameBoard = cCurrentGameBoard;
	// initialise players 
	CPlayer PlayerOne(&pCurrentGameBoard);
	CPlayer PlayerTwo(&pCurrentGameBoard);
	...
	while( !bQuitGame )
	{
		...
		while( cCurrentGameBoard.GetGameState() == g_iGameState )
		{			
			// swap players			
			g_iActivePlayer		= (k_iBoardSlot_Player1 == g_iActivePlayer ) ? PlayerTwo : PlayerOne;
...
//{
//	// reset board
//	memset( cCurrentGameBoard.g_aaiBoard, k_iBoardSlot_Empty, sizeof( g_aaiBoard) );
//
